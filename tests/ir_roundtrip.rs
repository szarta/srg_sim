...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 99
...
/// The exhaustive corpus must cover exactly the 99 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        99,
        "expected 99 distinct node types, got {}",
        tags.len()
    );
}
