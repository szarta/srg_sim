...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 97
...
/// The exhaustive corpus must cover exactly the 97 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        97,
        "expected 97 distinct node types, got {}",
        tags.len()
    );
}
