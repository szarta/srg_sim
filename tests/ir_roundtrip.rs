...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 167
...
/// The exhaustive corpus must cover exactly the 167 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        167,
        "expected 167 distinct node types, got {}",
        tags.len()
    );
}
