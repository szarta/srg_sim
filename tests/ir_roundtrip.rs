...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 163
...
/// The exhaustive corpus must cover exactly the 163 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        163,
        "expected 163 distinct node types, got {}",
        tags.len()
    );
}
