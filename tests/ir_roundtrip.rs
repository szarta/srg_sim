...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 162
...
/// The exhaustive corpus must cover exactly the 162 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        162,
        "expected 162 distinct node types, got {}",
        tags.len()
    );
}
