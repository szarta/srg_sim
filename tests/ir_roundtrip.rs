...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 70
...
/// The exhaustive corpus must cover exactly the 76 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        87,
        "expected 87 distinct node types, got {}",
        tags.len()
    );
}
