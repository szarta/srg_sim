...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 166
...
/// The exhaustive corpus must cover exactly the 166 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        166,
        "expected 166 distinct node types, got {}",
        tags.len()
    );
}
