...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 108
...
/// The exhaustive corpus must cover exactly the 108 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        108,
        "expected 108 distinct node types, got {}",
        tags.len()
    );
}
