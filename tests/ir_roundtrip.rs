...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 157
...
/// The exhaustive corpus must cover exactly the 157 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        157,
        "expected 157 distinct node types, got {}",
        tags.len()
    );
}
