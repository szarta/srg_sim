...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 110
...
/// The exhaustive corpus must cover exactly the 110 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        110,
        "expected 110 distinct node types, got {}",
        tags.len()
    );
}
