...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 105
...
/// The exhaustive corpus must cover exactly the 105 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        105,
        "expected 105 distinct node types, got {}",
        tags.len()
    );
}
