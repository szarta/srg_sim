...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 107
...
/// The exhaustive corpus must cover exactly the 107 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        107,
        "expected 107 distinct node types, got {}",
        tags.len()
    );
}
