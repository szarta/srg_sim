...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 172
...
/// The exhaustive corpus must cover exactly the 172 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        172,
        "expected 172 distinct node types, got {}",
        tags.len()
    );
}
