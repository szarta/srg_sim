...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 171
...
/// The exhaustive corpus must cover exactly the 171 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        171,
        "expected 171 distinct node types, got {}",
        tags.len()
    );
}
