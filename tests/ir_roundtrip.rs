...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 168
...
/// The exhaustive corpus must cover exactly the 168 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        168,
        "expected 168 distinct node types, got {}",
        tags.len()
    );
}
