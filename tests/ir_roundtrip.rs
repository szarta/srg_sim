...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 177
...
/// The exhaustive corpus must cover exactly the 177 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        177,
        "expected 177 distinct node types, got {}",
        tags.len()
    );
}
