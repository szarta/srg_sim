...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 159
...
/// The exhaustive corpus must cover exactly the 159 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        159,
        "expected 159 distinct node types, got {}",
        tags.len()
    );
}
