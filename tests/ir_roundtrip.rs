...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 109
...
/// The exhaustive corpus must cover exactly the 109 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        109,
        "expected 109 distinct node types, got {}",
        tags.len()
    );
}
