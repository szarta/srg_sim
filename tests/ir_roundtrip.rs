...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 174
...
/// The exhaustive corpus must cover exactly the 174 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        174,
        "expected 174 distinct node types, got {}",
        tags.len()
    );
}
