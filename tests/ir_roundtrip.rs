...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 160
...
/// The exhaustive corpus must cover exactly the 160 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        160,
        "expected 160 distinct node types, got {}",
        tags.len()
    );
}
