...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 156
...
/// The exhaustive corpus must cover exactly the 156 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        156,
        "expected 156 distinct node types, got {}",
        tags.len()
    );
}
