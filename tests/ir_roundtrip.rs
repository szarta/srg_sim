...
//!   * `fixtures/ir/all_nodes.json` — one schema-minimal instance of *all* 155
...
/// The exhaustive corpus must cover exactly the 155 node types in the schema —
...
fn all_nodes_covers_every_type() {
    ...
    assert_eq!(
        tags.len(),
        155,
        "expected 155 distinct node types, got {}",
        tags.len()
    );
}
