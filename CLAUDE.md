...
references. **Before modeling, grep [`docs/development/grammar-catalog.md`](docs/development/grammar-catalog.md)**
(generated by `invoke grammar-catalog`) to see whether the shape already parses —
grammar lands in one of the 13 domain sub-tables in `build_rules`, and the ~90
builder helpers are grouped into 16 concern sections above them. The full
clause → IR → engine map is [`docs/development/rule-pipeline.rst`](docs/development/rule-pipeline.rst);
the procedure and traps are [`docs/development/coverage-grind.rst`](docs/development/coverage-grind.rst).
