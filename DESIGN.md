...
GrantBreakoutBonus(delta)     # TIMED imperative "+delta to your breakout rolls until the end of the turn" (The Mailman Always Delivers) — accumulates onto the actor's breakout_bonus_eot store (added for the defender by breakout_bonus), swept at the next turn start. Unlike Static BreakoutModifier it survives the source card leaving play — needed because Mailman shuffles itself away as it grants the bonus. schema v128
