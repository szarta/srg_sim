...
ModifyRoll(who, delta, when=THIS|NEXT, per?, per_who=OPP, per_zone=IN_PLAY, per_crowd=False)  # delta scales by count of `per` cards
                              # per_crowd -> one-shot pending delta = the live Crowd Meter × `delta` as a signed coefficient,
                              #   snapshotted at apply time ("your opponent's next turn roll is - the Crowd Meter" = delta -1). schema v161
...
BuffSkill(skill, delta, who, duration=WHILE_IN_PLAY, target_highest?, target_lowest?, per_crowd?, cap?, per?, per_zone=IN_PLAY, per_excludes_self=False)
                                                 ...
                                                 # in per_zone {IN_PLAY|DISCARD|FLIPPED_THIS_TURN|HAND} matching per), clamped to cap
...
Unstoppable(by_order?, by_name?, by_skillreq=False, applies_name?, player_scope=False)  # Static self-decl: cannot be stopped by stops of `by_order`/named `by_name`/skill-req `by_skillreq` (None/False = anything). applies_name = player-scope "your cards with \"X\" in the name cannot be stopped", matched against the ATTACK not the stopper (schema v152). player_scope = "Your cards …" (vs "This card …"): covers every owner card, read even from an in-play main-deck source; "printed Finishes" is by_order=Finish since play_order is the printed order (schema v153)
...
PreventBreakout(who=SELF)  # HARD breakout prevention — the affected side gets ZERO breakout rolls, distinct from BreakoutAttempts (whose count floors at 1, so it can never zero out). A Static in-play standing marker gated by the effect's own condition (evaluated at breakout time — "When the Crowd Meter is 3 or greater, your opponent cannot breakout"; "If you have 10 Strikes in play, your opponent cannot break out"). who=OPP -> "your opponent cannot breakout" (the common case), SELF -> "you cannot breakout". Read by breakout_prevented, which short-circuits the whole breakout loop before any roll (schema v162)
FinishRollBonus(delta, when_skill?, either=False, per?, per_who=SELF, per_zone=IN_PLAY, per_divisor?, cap?, per_excludes_self=False, per_crowd=False)  # +delta to a Finish roll; when_skill gates on the rolled skill. either -> symmetric "if either player rolls <S> for their Finish roll", read from the opponent's board too (v107 consumes the field). `per` set = delta * floor((count of per_who's cards in per_zone matching the filter) / per_divisor) — "+1 per Spotlight in your opponent's discard" (schema v28); per_zone=FLIPPED_THIS_TURN for "+1 for each Strike card flipped", per_divisor=3 for "+1 for every 3 Strikes in play" (schema v74); per_zone=HAND for "+1 for every 4 cards in your hand" (GOAT's Dive Bomb, schema v151); cap clamps the per-count product ("(Max +2)"), per_excludes_self drops the source card ("for each OTHER <X> you have in play") — read via the source-threaded fold (schema v106). per_crowd -> a SECOND live-Crowd-Meter addend (clamped to cap), on top of the Crowd Meter the finish math already folds into every roll — "Your Finish roll is + the Crowd Meter (Max +N)" (schema v123). The addend is `delta × meter` where delta is a coefficient with 0 read as 1 (existing "+the" nodes keep delta 0); "+ Triple the Crowd Meter" = delta 3 (schema v161, shared crowd_scaled helper across all per_crowd readers)
TurnRollBonus(skill, delta, who=SelfSide, either=False, per_crowd=False, cap=None)   # Static self-decl: +delta to a TURN roll when the rolled skill == `skill` — "Your Power is +N during turn rolls". Read by turn_roll_bonus in the roll-off (parallel of FinishRollBonus/BreakoutModifier); phase-scoped, so it never touches finish rolls / stops / skill comparisons the way a plain BuffSkill would (schema v97). who -> whose roll, from the owner's POV: SelfSide = the owner's own; Opp = the owner's opponent's ("your opponent's <S> is -N during their turn rolls"); turn_roll_bonus sums a roller's own SelfSide mods with their opponent's Opp mods (v122). either -> symmetric "if either player rolls <S> for their turn roll, their roll is +/-N", picked up from the opponent's board too (v107). per_crowd -> dynamic delta = Crowd Meter × `delta` coefficient (0 read as 1), clamped to cap ("your <S> is + the Crowd Meter (Max +N) during your turn roll"; the "During your turn roll:" header re-scoping a per_crowd BuffSkill so it stays roll-off-only, v118; shared crowd_scaled helper v161)
CanStopUnstoppable(only_order?) # player-scope Static decl: ALL of the declarer's stops may stop an otherwise-
                              # unstoppable attack ("You can stop cards that cannot be stopped" — the standing
                              # twin of Stop.even_unstoppable). Read in card_can_stop via can_stop_unstoppable. schema v154.
                              # only_order narrows the bypass to attacks of that PRINTED play order ("Ignore any
                              # \"Cannot be stopped\" text on your opponent's Finish cards"); None = blanket. schema v156
