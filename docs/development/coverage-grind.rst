...
**Before modeling a shape, check whether it is already handled.** The generated
:file:`docs/development/grammar-catalog.md` lists every grammar rule in
precedence order, grouped by domain, each with its regex and real example clauses
from the card DB — so ``grep`` it for the phrasing before writing a new rule (the
same shape often already parses under a broader pattern). It is regenerated by
``invoke grammar-catalog`` and its rule inventory is gated by
:file:`tests/grammar_catalog.rs`; regenerate after any rule change. New grammar
lands in one of the 13 domain sub-tables in ``build_rules``
(:file:`src/parser.rs`), whose ``domain_tables`` list is the single source of rule
order. For the full clause → IR → engine map, see :doc:`rule-pipeline`.
