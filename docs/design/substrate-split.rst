...
.. list-table::
   ...
   * - #
     ...
     - Choice
   * - Pausability
     - how the engine suspends at a decision point
     - **Continuation engine** — a resumable state machine, ``resume(response) -> Step``
   * - Language
     - where the authoritative engine lives
     - **Rust**, compiled to every target (native + WASM)
   * - Scope
     - how much becomes Rust
     - **Everything below the line + the parser** — full single-language stack
   * - Python's fate
     - after Rust is live
     - Was a **transitional oracle**; **RETIRED** at Phase 2 (task #79). Rust regresses against frozen golden corpora (engine logs + parser)
   * - RNG
     - cross-engine determinism
     - **Portable PRNG** (splitmix64/PCG) in both languages → byte-identical logs
...
.. list-table::
   ...
   * - Domain model, enums
     - ``cards.py``
     - ``cards``
   * - Card DB / decklist load
     - ``loader.py``
     - ``loader``
   * - **Effect IR (§3)**
     - ``effects.py``, ``conditions.py``
     - ``ir`` (serde structs, JSON round-trip)
   * - **Text → IR + coverage**
     - ``rules_parser.py`` + ``overrides.yaml``
     - ``parser`` (emits ``cards.ir.json``)
   * - Game state + ``observable``
     - ``state.py``
     - ``state``
   * - Turn loop / executor / finish sequence
     - ``engine.py``
     - ``engine`` (**resumable state machine**)
   * - Ported finish/breakout math
     - ``finish.py``
     - ``finish``
   * - Ported skill-stop math
     - ``stops.py``
     - ``stops``
   * - Seeded RNG
     - ``rng.py``
   ...
   * - **Game-log schema (§8)**
     - ``gamelog.py``
     - ``gamelog`` (serde structs)
   * - "Who decides" seam
     - ``policy.py`` (ABC + Random/Heuristic/Replay/profiles)
     - ``policy`` (trait + impls)
...
.. list-table::
   ...
   * - Console CLI
     - ``cli.py``
     - Rust CLI over ``srg-core``
   * - MCP server
     - *(new)*
     - Rust binary wrapping ``session``
   * - Interactive human play
     - ``interactive.py``
     - a ``HumanPolicy`` consumer of ``session``
   * - Post-game review
     - ``review.py``
     - replay consumer of ``gamelog``
   * - Batch analysis
     - ``analysis.py``
     - drives ``Engine::play`` in parallel
   * - Matchup report (PDF/HTML)
     - ``report/``
     - Python tooling (or → web layer)
...
.. note::
   ...
   **Realized scope (task 75, then retired at Phase 2).** Steps 3–4 ran live against the
   oracle at ``~/data/srg_sim_python`` via ``invoke conformance`` (``tests/parser_parity.rs``
   over the whole DB, and ``tests/session.rs::snapshot_restores_at_every_boundary``). Step 2
   (whole-log parity) was **not** re-run against that oracle: the ``python`` branch kept
   ``random.Random`` (MT19937) rather than adopting the §5 splitmix64 stream, an accepted
   split — so the two engines' logs cannot be byte-identical at a shared seed. Log parity
   was therefore owned Phase-2-style by the frozen splitmix64 corpus, which Rust reproduces
   in ``tests/engine_conformance.rs``. The parser is RNG-independent, so step 3 was the
   genuine cross-language check. **Post-retirement:** ``invoke conformance`` and
   ``gen_cards_ir.py`` are gone; snapshot/replay parity (``session.rs``) and the frozen parser
   golden (``parser_parity.rs``, regenerated by ``srg cards-ir``) both run inside ``cargo test``.

**Phase 2 — freeze & deprecate. ✅ DONE (task #79).** Rust reached 100% top-96 rules
coverage (``unsupported == 0``) and passed Phase 1; the Python engine is **retired**. Its
outputs are **frozen** into (a) the golden-log corpus (``fixtures/conformance/``, replayed
by ``engine_conformance.rs``) and (b) the whole-DB parser golden
(``fixtures/parser/cards.ir.json``, cross-validated 6386/6386 against the oracle, now
regenerated from the Rust parser via ``srg cards-ir`` and checked by ``parser_parity.rs``).
Both run in ``cargo test`` with no Python. The old ``srg_sim_python`` checkout is archival;
``gen_cards_ir.py`` and ``invoke conformance`` were removed, and the override expander was
relocated in-repo (``scripts/srg_ir/``) so authoring no longer consults the oracle.
...
- **M-R3 — cutover & deprecate. ✅ DONE (task #79).** Rust passed conformance on the
  top-96 (100% coverage) → froze the golden corpora and **retired the Python engine**
  (§6 Phase 2). Build-time card/IR tooling stays Python (self-contained, in-repo); the
  full report tooling migrates into the web layer later.
