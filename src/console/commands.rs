...
use srg_core::parser::{
    clause_rule_hits, coverage, parse_text, rule_catalog, rule_index_json, CoverageRecord,
    CoverageReport,
};
...
/// `grammar-catalog` — regenerate the grammar catalog. Writes the DB-free rule
/// inventory (`index_out`, gated by tests/grammar_catalog.rs) and the readable
/// per-rule reference (`doc_out`) with real example clauses from the card DB, so a
/// rule author can check whether a shape is already handled before modeling it.
pub fn gen_grammar_catalog(cards: &Path, doc_out: &Path, index_out: &Path) -> Result<()> {
    let index = CardIndex::from_yaml(cards)?;
    let catalog = rule_catalog();
    // Per-rule DB usage: total occurrences (card-weighted) and up to K distinct
    // example clauses, attributed to the FIRST rule that matches (live semantics).
    const K: usize = 4;
    let mut counts = vec![0usize; catalog.len()];
    let mut examples: Vec<Vec<String>> = vec![Vec::new(); catalog.len()];
    let mut seen: HashSet<(usize, String)> = HashSet::new();
    for rec in index.records() {
        for (clause, hit) in clause_rule_hits(rules_text(rec)) {
            let Some(i) = hit else { continue };
            counts[i] += 1;
            if examples[i].len() < K && seen.insert((i, clause.clone())) {
                examples[i].push(clause);
            }
        }
    }

    std::fs::write(index_out, rule_index_json())
        .with_context(|| format!("write {}", index_out.display()))?;
    let doc = render_grammar_catalog(&catalog, &counts, &examples, cards);
    std::fs::write(doc_out, doc).with_context(|| format!("write {}", doc_out.display()))?;
    println!(
        "{} + {}: {} rules, {} DB clauses attributed",
        doc_out.display(),
        index_out.display(),
        catalog.len(),
...
/// Render the human-facing catalog markdown: rules grouped by domain, each with its
/// regex, DB coverage count, and a few real example clauses.
fn render_grammar_catalog(
    catalog: &[srg_core::parser::RuleInfo],
    counts: &[usize],
    examples: &[Vec<String>],
    cards: &Path,
) -> String {
    let mut out = String::new();
    out.push_str("# Grammar catalog\n\n");
    out.push_str(
        "<!-- GENERATED by `srg grammar-catalog` (`invoke grammar-catalog`) — do not edit by hand. -->\n\n",
    );
    out.push_str(&format!(
        "The parser's {} grammar rules (regex → Effect builder), in **precedence order** —\n\
         the FIRST matching rule wins (`match_grammar`). Check here whether a clause shape is\n\
         already handled before modeling a new one. Examples are real clauses from `{}` that\n\
    ...
    let mut cur_domain = "";
    for info in catalog {
        if info.domain != cur_domain {
            cur_domain = info.domain;
            let n = catalog.iter().filter(|r| r.domain == cur_domain).count();
            out.push_str(&format!(
                "## {}\n\n{}\n\n_{n} rules._\n\n",
                info.domain, info.description
            ));
        }
        let ex = &examples[info.index];
        let ex_str = if ex.is_empty() {
            "_(no DB example)_".to_owned()
        } else {
            ex.iter()
                .map(|e| format!("`{e}`"))
                .collect::<Vec<_>>()
        ...
        out.push_str(&format!(
            "- **#{}** `{}` — {}× · {}\n",
            info.index, info.pattern, counts[info.index], ex_str,
        ));
    }
    out
}
